package org.ruoyi.knowledge.ingestion.split;
...
public class CodeTextSplitter implements TextSplitter {
    ...
    public List<String> split(String content, String kid) {
        log.info("[CodeTextSplitter] 开始分块: kid={}, contentLength={}, contentIsNull={}", 
                kid, content != null ? content.length() : 0, content == null);
        
        if (content == null) {
            log.warn("[CodeTextSplitter] content为null，kid={}", kid);
            return new ArrayList<>();
        }
        
        String trimmedContent = content.trim();
        if (trimmedContent.isEmpty()) {
            log.warn("[CodeTextSplitter] content为空或只包含空白字符，kid={}, originalLength={}", kid, content.length());
            return new ArrayList<>();
        }
        
        KnowledgeInfoVo knowledgeInfoVo = knowledgeInfoService.queryByKid(kid);
        if (knowledgeInfoVo == null) {
            log.error("[CodeTextSplitter] 知识库不存在: kid={}", kid);
            return new ArrayList<>();
        }
        ...
        log.info("[CodeTextSplitter] 分块配置: kid={}, knowledgeSeparator={}, textBlockSize={}, overlapChar={}", 
                kid, knowledgeSeparator, textBlockSize, overlapChar);
        
        if (textBlockSize <= 0) {
            log.error("[CodeTextSplitter] textBlockSize配置无效: kid={}, textBlockSize={}", kid, textBlockSize);
            return new ArrayList<>();
        }
        if (overlapChar < 0) {
            overlapChar = 0;
        ...
        log.info("[CodeTextSplitter] 分块完成: kid={}, 最终chunks数量={}", kid, chunkList.size());
        if (chunkList.isEmpty()) {
            log.warn("[CodeTextSplitter] 分块结果为空: kid={}, contentLength={}, knowledgeSeparator={}, textBlockSize={}", 
