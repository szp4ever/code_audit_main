...
public class AttachProcessServiceImpl implements IAttachProcessService {
    ...
    public void updateStatus(String processId, ProcessingStatus status, Object statusData) {
        ...
        // 更新进度
        // 对于处理中状态（PARSING、CHUNKING、VECTORIZING），
        // 如果还没有开始处理（没有currentIndex），使用起始进度而不是结束进度
        // 这样用户看到的是从起始进度开始逐步增长，而不是从结束进度跳回起始进度
        int progress;
        boolean isProcessingStatus = status == ProcessingStatus.PARSING 
    ...
    /**
     * 获取阶段的基础进度（阶段开始时的进度）
     * 简化后的流水线：UPLOADING(0) → PARSING(5) → CHUNKING(25) → VECTORIZING(55) → COMPLETED(100)
     */
    private int getBaseProgress(ProcessingStatus status) {
        return switch (status) {
            ...
            // Moved to enrichment layer - see ItemSuggestionService
            // case MATCHING -> 25;
     ...
     * PARSING    5-25%  (20)
     * CHUNKING   25-55% (30)
     * VECTORIZING 55-100% (45)
     */
    private int getProgressRange(ProcessingStatus status) {
        return switch (status) {
            case UPLOADING -> 5;       // 0-5%
            case PARSING -> 20;        // 5-25%
            case CHUNKING -> 30;       // 25-55%
            case VECTORIZING -> 45;    // 55-100%
            case COMPLETED -> 0;
            case FAILED, CANCELLED -> 0;
            // Moved to enrichment layer - see ItemSuggestionService
            // case MATCHING -> 25;
            // case USER_REVIEW_MATCHING -> 10;
            // case CREATING_ITEMS -> 15;
            // case USER_REVIEW_ITEMS -> 10;
            default -> 0;
        };
    }
    ...
    // Moved to enrichment layer - see ItemSuggestionService
    // The confirmMatching flow (MATCHING → USER_REVIEW_MATCHING → CREATING_ITEMS) is no longer
    // part of the upload pipeline. Retained as commented-out code for reference.
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void confirmMatching(String processId, List<MatchingDecision> decisions) {
        throw new ServiceException("匹配确认已移至AI增强层，请使用 ItemSuggestionService");
        /*
        KnowledgeAttachProcess process = processMapper.selectById(Long.parseLong(processId));
        if (process == null) {
            throw new ServiceException("处理任务不存在: processId=" + processId);
    ...
    // Moved to enrichment layer - see ItemSuggestionService
    // The confirmItems flow (CREATING_ITEMS → USER_REVIEW_ITEMS → VECTORIZING) is no longer
    // part of the upload pipeline. Retained as commented-out code for reference.
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void confirmItems(String processId, List<ItemModification> modifications) {
        throw new ServiceException("条目确认已移至AI增强层，请使用 ItemSuggestionService");
        /*
        KnowledgeAttachProcess process = processMapper.selectById(Long.parseLong(processId));
        if (process == null) {
            throw new ServiceException("处理任务不存在: processId=" + processId);
    ...
    // Moved to enrichment layer - see ItemSuggestionService
    // updateFragmentsAndCreateItemsAsync was part of the old MATCHING → CREATING_ITEMS pipeline.
    /**
     * @deprecated 已移至 AI 增强层
     */
    @Deprecated
    @Async
    public void updateFragmentsAndCreateItemsAsync(String processId, String docId, List<MatchingDecision> decisions, 
        ...
        throw new ServiceException("此方法已移至AI增强层，请使用 ItemSuggestionService");
        /*
        try {
            // 检查处理任务是否还存在且未被取消
    ...
    // Moved to enrichment layer - see ItemSuggestionService
    // createItemsAsync was part of the old CREATING_ITEMS pipeline.
    /**
     * @deprecated 已移至 AI 增强层
     */
    @Deprecated
    ...
    /**
     * 验证状态转换是否允许
     * 简化后的流水线：UPLOADING → PARSING → CHUNKING → VECTORIZING → COMPLETED
     * 任何非终态 → FAILED / CANCELLED
     * FAILED → PARSING（允许重试）
     */
    private boolean canTransition(ProcessingStatus from, ProcessingStatus to) {
        // 委托给枚举自身的 canTransitionTo 方法
        return from.canTransitionTo(to);

        // Moved to enrichment layer - see ItemSuggestionService
        // 旧的状态转换规则（保留注释供参考）：
        // CHUNKING → MATCHING → USER_REVIEW_MATCHING → CREATING_ITEMS → USER_REVIEW_ITEMS → VECTORIZING
    }
    ...
    private ExtractionContext buildExtractionContext(String kid) {
        ...
        context.setAvailableVulnerabilityTypes(allCwes.stream()
            .map(vo -> {
                org.ruoyi.knowledge.cwe.domain.CweReference cwe = new org.ruoyi.knowledge.cwe.domain.CweReference();
