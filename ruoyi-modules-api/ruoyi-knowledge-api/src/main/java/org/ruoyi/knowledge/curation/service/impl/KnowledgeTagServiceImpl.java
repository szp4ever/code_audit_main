...
import org.ruoyi.knowledge.curation.convert.KnowledgeTagConvert;
...
public class KnowledgeTagServiceImpl implements IKnowledgeTagService {
    ...
    private final KnowledgeTagConvert knowledgeTagConvert;
    ...
    public Boolean insertByBo(KnowledgeTagBo bo) {
        ...
        KnowledgeTag add = knowledgeTagConvert.toEntity(bo);
    ...
    public Boolean updateByBo(KnowledgeTagBo bo) {
        ...
        KnowledgeTag update = knowledgeTagConvert.toEntity(bo);
     ...
     * 最佳实践：
     * 1. 同一标签重复添加到同一条目应幂等
     * 2. 使用数据库唯一索引 (item_uuid, tag_id) 保证并发安全
     * 3. 捕获 DuplicateKeyException 实现幂等
     *
     * @param itemUuid 条目UUID
     * @param tagId    标签ID
     * @return true: 操作成功（新添加或已存在）
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Boolean addTagToItem(String itemUuid, Long tagId) {
        // 先查询是否已存在关联
        org.ruoyi.knowledge.curation.domain.KnowledgeItemTag existing =
            knowledgeItemTagMapper.selectOne(
                Wrappers.<org.ruoyi.knowledge.curation.domain.KnowledgeItemTag>lambdaQuery()
                    .eq(org.ruoyi.knowledge.curation.domain.KnowledgeItemTag::getItemUuid, itemUuid)
                    .eq(org.ruoyi.knowledge.curation.domain.KnowledgeItemTag::getTagId, tagId)
                    .last("LIMIT 1")
            );
        if (existing != null) {
            return true;
        ...
        // 尝试插入
        org.ruoyi.knowledge.curation.domain.KnowledgeItemTag itemTag =
            new org.ruoyi.knowledge.curation.domain.KnowledgeItemTag();
     ...
     * <p>
     * 最佳实践：
     * 1. 删除操作天然幂等 - 无论标签是否已关联，最终状态都是"未关联"
     * 2. 不应因记录不存在而返回失败
     *
     * @param itemUuid 条目UUID
     * @param tagId    标签ID
